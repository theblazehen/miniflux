...
func ProcessFeedEntries(store *storage.Storage, feed *model.Feed, userID int64, forceRefresh bool) {
	...
	for _, entry := range slices.Backward(feed.Entries) {
		...
		// Reading time must be calculated before the final filter pass so that
		// EntryReadingTime rules have a value to compare against.
		updateEntryReadingTime(store, feed, entry, entryIsNew, user)

		// Re-run filters after scraping, rewriting, and reading time calculation.
		if filter.IsBlockedEntry(blockRules, allowRules, feed, entry) {
			slog.Debug("Entry is blocked by filter rules",
				slog.Int64("user_id", user.ID),
				slog.String("entry_url", entry.URL),
				...
				slog.Int64("feed_id", feed.ID),
				slog.String("feed_url", feed.FeedURL),
				slog.String("filter_stage", "after_processing"),
			)
			continue
		}
